...
exports.CryptoConnection = exports.Connection = void 0;
...
class Connection extends events_1.EventEmitter {
    constructor(stream, options) {
        ...
        stream.on('close', () => {
            ...
            this.closed = true;
            this[kQueue].forEach(op => op.cb(new error_1.MongoNetworkError(`connection ${this.id} to ${this.address} closed`)));
            this[kQueue].clear();
            this.emit('close');
        });
        stream.on('timeout', () => {
            ...
            stream.destroy();
            this.closed = true;
            this[kQueue].forEach(op => op.cb(new error_1.MongoNetworkTimeoutError(`connection ${this.id} to ${this.address} timed out`, {
    ...
    getMore(ns, cursorId, options, callback) {
        ...
        if (!cursorId) {
            callback(new error_1.MongoError('Invalid internal cursor state, no known cursor id'));
            return;
        }
        if (wireVersion < 4) {
    ...
    killCursors(ns, cursorIds, options, callback) {
        ...
        this.command(ns, { killCursors: ns.collection, cursors: cursorIds }, { fullResult: true, ...options }, (err, response) => {
            ...
            if (!Array.isArray(response.documents) || response.documents.length === 0) {
                return callback(new error_1.MongoError(`invalid killCursors result returned for cursor id ${cursorIds[0]}`));
            }
...
class CryptoConnection extends Connection {
    ...
    command(ns, cmd, options, callback) {
        ...
        if (!autoEncrypter) {
            return callback(new error_1.MongoError('No AutoEncrypter available for encryption'));
        }
        ...
        if (serverWireVersion < 8) {
            callback(new error_1.MongoError('Auto-encryption requires a minimum MongoDB version of 4.2'));
            return;
        }
...
function supportsVersionedApi(cmd, session) {
    const inTransaction = session && (session.inTransaction() || transactions_1.isTransactionCommand(cmd));
    ...
    return ((!inTransaction || (session === null || session === void 0 ? void 0 : session.transaction.isStarting)) && !cmd.commitTransaction && !cmd.getMore);
}
function messageHandler(conn) {
    return function messageHandler(message) {
        ...
        try {
            ...
            callback(new error_1.MongoError(err));
            return;
        }
        if (message.documents[0]) {
            ...
            if (operationDescription.command) {
                ...
                if (document.ok === 0 || document.$err || document.errmsg || document.code) {
                    callback(new error_1.MongoError(document));
                    return;
                }
            }
            else {
