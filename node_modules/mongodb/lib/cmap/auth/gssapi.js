...
class GSSAPI extends auth_provider_1.AuthProvider {
    auth(authContext, callback) {
        ...
        if (credentials == null)
            return callback(new error_1.MongoError('credentials required'));
        ...
        makeKerberosClient(authContext, (err, client) => {
            if (err)
                return callback(err);
            if (client == null)
                return callback(new error_1.MongoError('gssapi client missing'));
...
function makeKerberosClient(authContext, callback) {
    ...
    if (!hostAddress || typeof hostAddress.host !== 'string' || !credentials) {
        return callback(new error_1.MongoError('Connection must have host and port and credentials defined.'));
    }
    ...
    const serviceName = mechanismProperties['gssapiservicename'] ||
    ...
    performGssapiCanonicalizeHostName(hostAddress.host, mechanismProperties, (err, host) => {
        ...
        deps_1.Kerberos.initializeClient(`${serviceName}${process.platform === 'win32' ? '/' : '@'}${host}`, initOptions, (err, client) => {
            if (err)
                return callback(new error_1.MongoError(err));
