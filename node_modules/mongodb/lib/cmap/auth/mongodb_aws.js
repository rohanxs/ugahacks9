...
class MongoDBAWS extends auth_provider_1.AuthProvider {
    auth(authContext, callback) {
        ...
        if (!credentials) {
            return callback(new error_1.MongoError('AuthContext must provide credentials.'));
        }
        if ('kModuleError' in deps_1.aws4) {
            return callback(deps_1.aws4['kModuleError']);
        }
        if (utils_1.maxWireVersion(connection) < 9) {
            callback(new error_1.MongoError('MONGODB-AWS authentication requires MongoDB version 4.4 or later'));
            return;
        }
        ...
        crypto.randomBytes(32, (err, nonce) => {
            ...
            connection.command(utils_1.ns(`${db}.$cmd`), saslStart, undefined, (err, res) => {
                ...
                if (serverNonce.length !== 64) {
                    callback(new error_1.MongoError(`Invalid server nonce length ${serverNonce.length}, expected 64`));
                    return;
                }
                if (serverNonce.compare(nonce, 0, nonce.length, 0, nonce.length) !== 0) {
                    callback(new error_1.MongoError('Server nonce does not begin with client nonce'));
                    return;
                }
                if (host.length < 1 || host.length > 255 || host.indexOf('..') !== -1) {
                    callback(new error_1.MongoError(`Server returned an invalid host: "${host}"`));
                    return;
                }
                const body = 'Action=GetCallerIdentity&Version=2011-06-15';
                const options = deps_1.aws4.sign({
                    method: 'POST',
                    host,
...
function makeTempCredentials(credentials, callback) {
    function done(creds) {
        if (creds.AccessKeyId == null || creds.SecretAccessKey == null || creds.Token == null) {
            callback(new error_1.MongoError('Could not obtain temporary MONGODB-AWS credentials'));
            return;
        }
    ...
    const req = http.request(options, res => {
        ...
        res.on('end', () => {
            ...
            try {
                ...
                callback(new error_1.MongoError(`Invalid JSON response: "${data}"`));
            }
        });
    });
