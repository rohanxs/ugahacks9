...
function executeScram(cryptoMethod, authContext, callback) {
    ...
    if (!authContext.nonce) {
        return callback(new error_1.MongoError('AuthContext must contain a valid nonce property'));
    }
...
function continueScramConversation(cryptoMethod, response, authContext, callback) {
    ...
    if (!authContext.nonce) {
        return callback(new error_1.MongoError('Unable to continue SCRAM without valid nonce'));
    }
    const nonce = authContext.nonce;
    const db = credentials.source;
    const username = cleanUsername(credentials.username);
    ...
    if (iterations && iterations < 4096) {
        callback(new error_1.MongoError(`Server returned an invalid iteration count ${iterations}`), false);
        return;
    }
    const salt = dict.s;
    const rnonce = dict.r;
    if (rnonce.startsWith('nonce')) {
        callback(new error_1.MongoError(`Server returned an invalid nonce: ${rnonce}`), false);
        return;
    }
    ...
    connection.command(utils_1.ns(`${db}.$cmd`), saslContinueCmd, undefined, (_err, r) => {
        ...
        if (!compareDigest(Buffer.from(parsedResponse.v, 'base64'), serverSignature)) {
            callback(new error_1.MongoError('Server returned an invalid signature'));
            return;
        }
        if (!r || r.done !== false) {
...
function resolveError(err, result) {
    ...
    if (result) {
        if (result.$err || result.errmsg)
            return new error_1.MongoError(result);
    }
}
