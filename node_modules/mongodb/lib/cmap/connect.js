...
function checkSupportedServer(ismaster, options) {
    ...
    if (serverVersionHighEnough) {
        ...
        return new error_1.MongoError(message);
    }
...
function performInitialHandshake(conn, options, _callback) {
    ...
    if (credentials) {
        if (!(credentials.mechanism === defaultAuthProviders_1.AuthMechanism.MONGODB_DEFAULT) &&
            ...
            callback(new error_1.MongoError(`authMechanism '${credentials.mechanism}' not supported`));
            return;
        }
    }
    ...
    prepareHandshakeDocument(authContext, (err, handshakeDoc) => {
        ...
        conn.command(utils_1.ns('admin.$cmd'), handshakeDoc, handshakeOptions, (err, response) => {
            ...
            if ((response === null || response === void 0 ? void 0 : response.ok) === 0) {
                callback(new error_1.MongoError(response));
                return;
            }
            ...
            if (!response.arbiterOnly && credentials) {
                ...
                if (!provider) {
                    return callback(new error_1.MongoError(`No AuthProvider for ${resolvedCredentials.mechanism} defined.`));
                }
    ...
    if (credentials) {
        if (credentials.mechanism === defaultAuthProviders_1.AuthMechanism.MONGODB_DEFAULT && credentials.username) {
            ...
            if (!provider) {
                ...
                return callback(new error_1.MongoError(`No AuthProvider for ${defaultAuthProviders_1.AuthMechanism.MONGODB_SCRAM_SHA256} defined.`));
            }
            return provider.prepare(handshakeDoc, authContext, callback);
        }
        const provider = defaultAuthProviders_1.AUTH_PROVIDERS.get(credentials.mechanism);
        if (!provider) {
            return callback(new error_1.MongoError(`No AuthProvider for ${credentials.mechanism} defined.`));
        }
        return provider.prepare(handshakeDoc, authContext, callback);
    }
...
function parseConnectOptions(options) {
    ...
    if (!hostAddress)
        throw new Error('HostAddress required');
    const result = {};
