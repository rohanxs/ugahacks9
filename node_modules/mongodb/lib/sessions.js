...
function assertAlive(session, callback) {
    if (session.serverSession == null) {
        const error = new error_1.MongoError('Cannot use a session that has ended');
 ...
 * @public
 */
class ClientSession extends events_1.EventEmitter {
    /**
    ...
    startTransaction(options) {
        ...
        if (this.inTransaction()) {
            throw new error_1.MongoError('Transaction already in progress');
        }
        const topologyMaxWireVersion = utils_1.maxWireVersion(this.topology);
        if (shared_1.isSharded(this.topology) &&
            topologyMaxWireVersion != null &&
            topologyMaxWireVersion < minWireVersionForShardedTransactions) {
            throw new error_1.MongoError('Transactions are not supported on sharded clusters in MongoDB < 4.2.');
        }
        // increment txnNumber
...
function endTransaction(session, commandName, callback) {
    ...
    if (txnState === transactions_1.TxnState.NO_TRANSACTION) {
        callback(new error_1.MongoError('No transaction started'));
        return;
    }
    if (commandName === 'commitTransaction') {
        ...
        if (txnState === transactions_1.TxnState.TRANSACTION_ABORTED) {
            callback(new error_1.MongoError('Cannot call commitTransaction after calling abortTransaction'));
            return;
        }
    }
    else {
        ...
        if (txnState === transactions_1.TxnState.TRANSACTION_ABORTED) {
            callback(new error_1.MongoError('Cannot call abortTransaction twice'));
            return;
        }
        if (txnState === transactions_1.TxnState.TRANSACTION_COMMITTED ||
            txnState === transactions_1.TxnState.TRANSACTION_COMMITTED_EMPTY) {
            callback(new error_1.MongoError('Cannot call abortTransaction after calling commitTransaction'));
            return;
        }
    }
    ...
    function commandHandler(e, r) {
        if (commandName === 'commitTransaction') {
            ...
            if (e &&
                (e instanceof error_1.MongoNetworkError ||
...
function applySession(session, command, options) {
    ...
    if (session.hasEnded) {
        return new error_1.MongoError('Attemped to use a session that has ended');
    }
    const serverSession = session.serverSession;
    if (serverSession == null) {
        return new error_1.MongoError('Unable to acquire server session');
    }
    ...
    if (options && options.writeConcern && options.writeConcern.w === 0) {
        if (session && session.explicit) {
            return new error_1.MongoError('Cannot have explicit session with unacknowledged writes');
        }
        return;
    }
