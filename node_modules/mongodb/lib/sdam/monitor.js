...
class Monitor extends events_1.EventEmitter {
    constructor(server, options) {
        ...
        this[kCancellationToken] = new events_1.EventEmitter();
...
function checkServer(monitor, callback) {
    ...
    monitor.emit(server_1.Server.SERVER_HEARTBEAT_STARTED, new events_2.ServerHeartbeatStartedEvent(monitor.address));
    function failureHandler(err) {
        var _a;
        (_a = monitor[kConnection]) === null || _a === void 0 ? void 0 : _a.destroy({ force: true });
        monitor[kConnection] = undefined;
        monitor.emit(server_1.Server.SERVER_HEARTBEAT_FAILED, new events_2.ServerHeartbeatFailedEvent(monitor.address, utils_1.calculateDurationInMs(start), err));
    ...
    if (connection && !connection.closed) {
        ...
        const cmd = isAwaitable && topologyVersion
        ...
        connection.command(utils_1.ns('admin.$cmd'), cmd, options, (err, isMaster) => {
            ...
            monitor.emit(server_1.Server.SERVER_HEARTBEAT_SUCCEEDED, new events_2.ServerHeartbeatSucceededEvent(monitor.address, duration, isMaster));
    ...
    connect_1.connect(monitor.connectOptions, (err, conn) => {
        ...
        if (conn) {
            ...
            monitor.emit(server_1.Server.SERVER_HEARTBEAT_SUCCEEDED, new events_2.ServerHeartbeatSucceededEvent(monitor.address, utils_1.calculateDurationInMs(start), conn.ismaster));
