...
class Server extends events_1.EventEmitter {
    ...
    constructor(topology, description, options) {
        ...
        utils_1.relayEvents(this.s.pool, this, ['commandStarted', 'commandSucceeded', 'commandFailed'].concat(connection_pool_1.CMAP_EVENT_NAMES));
        ...
        utils_1.relayEvents(this[kMonitor], this, [
    ...
    command(ns, cmd, options, callback) {
        ...
        if (this.s.state === common_1.STATE_CLOSING || this.s.state === common_1.STATE_CLOSED) {
            callback(new error_1.MongoError('server is closed'));
            return;
        }
        // Clone the options
        ...
        if (this.s.logger.isDebug()) {
            this.s.logger.debug(`executing command [${JSON.stringify({
                ns,
                cmd,
                options: utils_1.debugOptions(DEBUG_FIELDS, options)
            })}] against ${this.name}`);
        }
        // error if collation not supported
        if (utils_1.collationNotSupported(this, cmd)) {
            callback(new error_1.MongoError(`server ${this.name} does not support collation`));
            return;
        }
