 ...
 * @public
 */
class Topology extends events_1.EventEmitter {
    /**
    ...
    constructor(seeds, options) {
        ...
        if (options.srvHost) {
            ...
            this.s.detectTopologyDescriptionChange = (ev) => {
                ...
                if (previousType !== common_1.TopologyType.Sharded && newType === common_1.TopologyType.Sharded) {
    ...
    connect(options, callback) {
        ...
        this.emit(Topology.TOPOLOGY_OPENING, new events_2.TopologyOpeningEvent(this.s.id));
        // emit an event for the topology change
        this.emit(Topology.TOPOLOGY_DESCRIPTION_CHANGED, new events_2.TopologyDescriptionChangedEvent(this.s.id, new topology_description_1.TopologyDescription(common_1.TopologyType.Unknown), // initial is always Unknown
        this.s.description));
    ...
    close(options, callback) {
        ...
        drainWaitQueue(this[kWaitQueue], new error_1.MongoError('Topology closed'));
        ...
        this.s.sessionPool.endAllPooledSessions(() => {
            utils_1.eachAsync(Array.from(this.s.servers.values()), (server, cb) => destroyServer(server, this, options, cb), err => {
                ...
                this.emit(Topology.TOPOLOGY_CLOSED, new events_2.TopologyClosedEvent(this.s.id));
    ...
    serverUpdateHandler(serverDescription) {
        ...
        if (this.s.description.compatibilityError) {
            this.emit(Topology.ERROR, new error_1.MongoError(this.s.description.compatibilityError));
            return;
        }
        ...
        if (!equalDescriptions) {
            ...
            if (newDescription) {
                this.emit(Topology.SERVER_DESCRIPTION_CHANGED, new events_2.ServerDescriptionChangedEvent(this.s.id, serverDescription.address, previousServerDescription, newDescription));
            }
        }
        ...
        if (!equalDescriptions) {
            this.emit(Topology.TOPOLOGY_DESCRIPTION_CHANGED, new events_2.TopologyDescriptionChangedEvent(this.s.id, previousTopologyDescription, this.s.description));
        }
    }
...
function destroyServer(server, topology, options, callback) {
    ...
    server.destroy(options, () => {
        topology.emit(Topology.SERVER_CLOSED, new events_2.ServerClosedEvent(topology.s.id, server.description.address));
...
function createAndConnectServer(topology, serverDescription, connectDelay) {
    topology.emit(Topology.SERVER_OPENING, new events_2.ServerOpeningEvent(topology.s.id, serverDescription.address));
    const server = new server_1.Server(topology, serverDescription, topology.s.options);
    utils_1.relayEvents(server, topology, SERVER_RELAY_EVENTS);
    server.on(server_1.Server.DESCRIPTION_RECEIVED, topology.serverUpdateHandler.bind(topology));
    if (connectDelay) {
...
function srvPollingHandler(topology) {
    return function handleSrvPolling(ev) {
        ...
        topology.emit(Topology.SERVER_DESCRIPTION_CHANGED, new events_2.TopologyDescriptionChangedEvent(topology.s.id, previousTopologyDescription, topology.s.description));
    };
}
...
function processWaitQueue(topology) {
    if (topology.s.state === common_1.STATE_CLOSED) {
        drainWaitQueue(topology[kWaitQueue], new error_1.MongoError('Topology is closed, please connect'));
        return;
    }
