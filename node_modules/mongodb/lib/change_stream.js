...
const NO_RESUME_TOKEN_ERROR = new error_1.MongoError('A change stream document has been received that lacks a resume token (_id).');
const NO_CURSOR_ERROR = new error_1.MongoError('ChangeStream has no cursor');
const CHANGESTREAM_CLOSED_ERROR = new error_1.MongoError('ChangeStream is closed');
/**
 * Creates a new Change Stream instance. Normally created using {@link Collection#watch|Collection.watch()}.
 * @public
 */
class ChangeStream extends events_1.EventEmitter {
    /**
     * @internal
     *
...
function createChangeStreamCursor(changeStream, options) {
    ...
    utils_1.relayEvents(changeStreamCursor, changeStream, ['resumeTokenChanged', 'end', 'close']);
...
function waitForTopologyConnected(topology, options, callback) {
    setTimeout(() => {
        ...
        if (utils_1.calculateDurationInMs(start) > timeout) {
            return callback(new error_1.MongoError('Timed out waiting for connection'));
        }
