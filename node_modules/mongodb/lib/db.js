...
const dynamic_loaders_1 = require("./dynamic_loaders");
...
class Db {
    ...
    admin() {
        const AdminClass = dynamic_loaders_1.loadAdmin();
    ...
    collection(name, options, callback) {
        ...
        if (!finalOptions.strict) {
            try {
                const collection = new collection_1.Collection(this, name, finalOptions);
                if (callback)
                    callback(undefined, collection);
                return collection;
            }
            catch (err) {
                if (err instanceof error_1.MongoError && callback)
                    return callback(err);
                throw err;
            }
        }
        ...
        if (typeof callback !== 'function') {
            throw new error_1.MongoError(`A callback is required in strict mode. While getting collection ${name}`);
        }
        // Did the user destroy the topology
        if (utils_1.getTopology(this).isDestroyed()) {
            return callback(new error_1.MongoError('topology was destroyed'));
        }
        ...
        this.listCollections({ name }, listCollectionOptions).toArray((err, collections) => {
            if (callback == null)
                return;
            if (err != null || !collections)
                return callback(err);
            if (collections.length === 0)
                return callback(new error_1.MongoError(`Collection ${name} does not exist. Currently in strict mode.`));
            try {
                return callback(undefined, new collection_1.Collection(this, name, finalOptions));
            }
            catch (err) {
                return callback(err);
            }
...
function validateDatabaseName(databaseName) {
    if (typeof databaseName !== 'string')
        throw new error_1.MongoError('database name must be a string');
    if (databaseName.length === 0)
        throw new error_1.MongoError('database name cannot be the empty string');
    if (databaseName === '$external')
        return;
    const invalidChars = [' ', '.', '$', '/', '\\'];
    for (let i = 0; i < invalidChars.length; i++) {
        if (databaseName.indexOf(invalidChars[i]) !== -1)
            throw new error_1.MongoError(`database names cannot contain the character '${invalidChars[i]}'`);
    }
}
