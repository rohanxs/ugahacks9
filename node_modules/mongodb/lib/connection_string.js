...
exports.DEFAULT_OPTIONS = exports.OPTIONS = exports.parseOptions = exports.parseURI = exports.checkTLSOptions = exports.resolveSRVRecord = void 0;
...
function parseURI(uri) {
    ...
    if (!match) {
        throw new error_1.MongoParseError(`Invalid connection string ${uri}`);
    }
    const protocol = (_a = match.groups) === null || _a === void 0 ? void 0 : _a.protocol;
    const username = (_b = match.groups) === null || _b === void 0 ? void 0 : _b.username;
    const password = (_c = match.groups) === null || _c === void 0 ? void 0 : _c.password;
    const hosts = (_d = match.groups) === null || _d === void 0 ? void 0 : _d.hosts;
    const rest = (_e = match.groups) === null || _e === void 0 ? void 0 : _e.rest;
    if (!protocol || !hosts) {
        throw new error_1.MongoParseError('Invalid connection string, protocol and host(s) required');
    }
    ...
    if (username === null || username === void 0 ? void 0 : username.match(illegalCharacters)) {
        throw new error_1.MongoParseError(`Username contains unescaped characters ${username}`);
    }
    if (!username || !password) {
        const uriWithoutProtocol = uri.replace(`${protocol}://`, '');
        if (uriWithoutProtocol.startsWith('@') || uriWithoutProtocol.startsWith(':')) {
            throw new error_1.MongoParseError('URI contained empty userinfo section');
        }
    }
    if (password === null || password === void 0 ? void 0 : password.match(illegalCharacters)) {
        throw new error_1.MongoParseError('Password contains unescaped characters');
    }
    let authString = '';
    ...
    const url = new url_1.URL(`${protocol.toLowerCase()}://${authString}@dummyHostname${rest}`);
    if (isSRV && hostList.length !== 1) {
        throw new error_1.MongoParseError('mongodb+srv URI cannot have multiple service names');
    }
    if (isSRV && hostList[0].includes(':')) {
        throw new error_1.MongoParseError('mongodb+srv URI cannot have port number');
    }
    return {
        isSRV,
        url,
...
function parseOptions(uri, mongoClient = undefined, options = {}) {
    ...
    if (unsupportedOptions.size !== 0) {
        throw new error_1.MongoParseError(`options ${Array.from(unsupportedOptions).join(', ')} are not supported`);
    }
    ...
    if (mongoClient && options.autoEncryption) {
        mongoOptions.autoEncrypter = connect_1.createAutoEncrypter(mongoClient, mongoOptions);
    }
    ...
    tlsInsecure: {
        transform({ name, options, values: [value] }) {
            ...
            if (tlsInsecure) {
                ...
                options.checkServerIdentity = options.tlsAllowInvalidHostnames ? undefined : true;
