...
function checkDone(stream, callback) {
    ...
    if (stream.state.streamEnd && stream.state.outstandingRequests === 0 && !stream.state.errored) {
        ...
        const filesDoc = createFilesDoc(stream.id, stream.length, stream.chunkSizeBytes, stream.md5 && stream.md5.digest('hex'), stream.filename, stream.options.contentType, stream.options.aliases, stream.options.metadata);
