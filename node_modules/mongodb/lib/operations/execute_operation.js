    ...
    return utils_1.maybePromise(callback, cb => {
        ...
        if (topology.hasSessionSupport()) {
            if (session == null) {
                ...
                return cb(new error_1.MongoError('Use of expired sessions is not permitted'));
            }
        }
        else if (session) {
            // If the user passed an explicit session and we are still, after server selection,
            // trying to run against a topology that doesn't support sessions we error out.
            return cb(new error_1.MongoError('Current topology does not support sessions'));
        }
        try {
...
function executeWithServerSelection(topology, session, operation, callback) {
    ...
    if (inTransaction && !readPreference.equals(read_preference_1.ReadPreference.primary)) {
        callback(new error_1.MongoError(`Read preference in a transaction must be primary, not: ${readPreference.mode}`));
        return;
    }
    ...
    function callbackWithRetry(err, result) {
        ...
        if (hasWriteAspect &&
            ...
            callback(new error_1.MongoError({
