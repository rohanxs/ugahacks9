...
class FindOperation extends command_1.CommandOperation {
    constructor(collection, ns, filter = {}, options = {}) {
        ...
        if (typeof filter !== 'object' || Array.isArray(filter)) {
            throw new error_1.MongoError('Query filter must be a plain object or ObjectId');
        }
        ...
        if (Buffer.isBuffer(filter)) {
            ...
            if (objectSize !== filter.length) {
                throw new TypeError(`query filter raw message size does not match message header size [${filter.length}] != [${objectSize}]`);
            }
        }
    ...
    execute(server, session, callback) {
        ...
        if (typeof options.allowDiskUse !== 'undefined' && serverWireVersion < 4) {
            callback(new error_1.MongoError('The `allowDiskUse` option is not supported on MongoDB < 3.2'));
            return;
        }
        if (options.collation && serverWireVersion < SUPPORTS_WRITE_CONCERN_AND_COLLATION) {
            callback(new error_1.MongoError(`Server ${server.name}, which reports wire version ${serverWireVersion}, does not support collation`));
            return;
        }
        if (serverWireVersion < 4) {
            if (this.readConcern && this.readConcern.level !== 'local') {
                callback(new error_1.MongoError(`server find command does not support a readConcern level of ${this.readConcern.level}`));
                return;
            }
