...
class UpdateOperation extends command_1.CommandOperation {
    ...
    execute(server, session, callback) {
        ...
        if (utils_1.collationNotSupported(server, options) ||
            ...
            callback(new error_1.MongoError(`server ${server.name} does not support collation`));
            return;
        }
        const unacknowledgedWrite = this.writeConcern && this.writeConcern.w === 0;
        if (unacknowledgedWrite || utils_1.maxWireVersion(server) < 5) {
            if (this.statements.find((o) => o.hint)) {
                callback(new error_1.MongoError(`servers < 3.4 do not support hint on update`));
                return;
            }
        }
        if (this.explain && utils_1.maxWireVersion(server) < 3) {
            callback(new error_1.MongoError(`server ${server.name} does not support explain on update`));
            return;
        }
        if (this.statements.some(statement => !!statement.arrayFilters) && utils_1.maxWireVersion(server) < 6) {
            callback(new error_1.MongoError('arrayFilters are only supported on MongoDB 3.6+'));
            return;
        }
...
class UpdateOneOperation extends UpdateOperation {
    ...
    execute(server, session, callback) {
        super.execute(server, session, (err, res) => {
            ...
            if (res.code)
                return callback(new error_1.MongoError(res));
            if (res.writeErrors)
                return callback(new error_1.MongoError(res.writeErrors[0]));
            callback(undefined, {
