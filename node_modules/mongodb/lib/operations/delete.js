...
class DeleteOperation extends command_1.CommandOperation {
    ...
    execute(server, session, callback) {
        ...
        if (options.explain !== undefined && utils_1.maxWireVersion(server) < 3) {
            return callback
                ? callback(new error_1.MongoError(`server ${server.name} does not support explain on delete`))
                : undefined;
        }
        const unacknowledgedWrite = this.writeConcern && this.writeConcern.w === 0;
        if (unacknowledgedWrite || utils_1.maxWireVersion(server) < 5) {
            if (this.statements.find((o) => o.hint)) {
                callback(new error_1.MongoError(`servers < 3.4 do not support hint on delete`));
                return;
            }
        }
