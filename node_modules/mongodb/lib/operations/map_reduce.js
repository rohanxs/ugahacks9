...
class MapReduceOperation extends command_1.CommandOperation {
    ...
    execute(server, session, callback) {
        ...
        if (this.explain && utils_1.maxWireVersion(server) < 9) {
            callback(new error_1.MongoError(`server ${server.name} does not support explain on mapReduce`));
            return;
        }
        // Execute command
        super.executeCommand(server, session, mapCommandHash, (err, result) => {
            ...
            if (1 !== result.ok || result.err || result.errmsg) {
                return callback(new error_1.MongoError(result));
            }
            ...
            if (result.result != null && typeof result.result === 'object') {
                ...
                const Db = dynamic_loaders_1.loadDb();
                collection = new Db(coll.s.db.s.client, doc.db, coll.s.db.s.options).collection(doc.collection);
            }
            else {
