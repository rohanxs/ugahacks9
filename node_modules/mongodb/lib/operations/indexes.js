    ...
    execute(server, session, callback) {
        ...
        for (let i = 0; i < indexes.length; i++) {
            ...
            if (indexes[i].collation && serverWireVersion < 5) {
                callback(new error_1.MongoError(`Server ${server.name}, which reports wire version ${serverWireVersion}, ` +
        ...
        if (options.commitQuorum != null) {
            if (serverWireVersion < 9) {
                callback(new error_1.MongoError('`commitQuorum` option for `createIndexes` not supported on servers < 4.4'));
                return;
            }
