...
class BulkWriteResult {
    ...
    getWriteConcernError() {
        if (this.result.writeConcernErrors.length === 0) {
            ...
            return new WriteConcernError(new error_1.MongoError({ errmsg: errmsg, code: error_1.MONGODB_ERROR_CODES.WriteConcernFailed }));
        }
    }
    toJSON() {
        return this.result;
...
function handleMongoWriteConcernError(batch, bulkResult, err, callback) {
    ...
    const wrappedWriteConcernError = new WriteConcernError(new error_1.MongoError({
    ...
    callback(new MongoBulkWriteError(new error_1.MongoError(wrappedWriteConcernError), new BulkWriteResult(bulkResult)));
}
/**
 ...
 * @category Error
 */
class MongoBulkWriteError extends error_1.MongoError {
...
class BulkOperationBase {
    ...
    raw(op) {
        ...
        if ('removeOne' in op) {
            return this.addToOperationsList(exports.BatchType.DELETE, delete_1.makeDeleteStatement(op.removeOne.filter, { ...op.removeOne, limit: 1 }));
        }
        if ('removeMany' in op) {
            return this.addToOperationsList(exports.BatchType.DELETE, delete_1.makeDeleteStatement(op.removeMany.filter, { ...op.removeMany, limit: 0 }));
        }
        if ('deleteOne' in op) {
        ...
        throw TypeError('bulkWrite only supports insertOne, insertMany, updateOne, updateMany, removeOne, removeMany, deleteOne, deleteMany');
    }
    get bsonOptions() {
    ...
    execute(options, callback) {
        ...
        if (this.s.executed) {
            return handleEarlyError(new error_1.MongoError('Batch cannot be re-executed'), callback);
        }
    ...
    handleWriteError(callback, writeResult) {
        if (this.s.bulkResult.writeErrors.length > 0) {
            ...
            callback(new MongoBulkWriteError(new error_1.MongoError({
                message: msg,
        ...
        if (writeConcernError) {
            callback(new MongoBulkWriteError(new error_1.MongoError(writeConcernError), writeResult));
            return true;
        }
    }
}
