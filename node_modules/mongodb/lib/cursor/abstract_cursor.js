...
/** @public */
class AbstractCursor extends events_1.EventEmitter {
    ...
    next(callback) {
        return utils_1.maybePromise(callback, done => {
            if (this[kId] === bson_1.Long.ZERO) {
                return done(new error_1.MongoError('Cursor is exhausted'));
            }
            next(this, true, done);
        });
    }
    ...
    addCursorFlag(flag, value) {
        ...
        if (!exports.CURSOR_FLAGS.includes(flag)) {
            throw new error_1.MongoError(`flag ${flag} is not one of ${exports.CURSOR_FLAGS}`);
        }
        if (typeof value !== 'boolean') {
            throw new error_1.MongoError(`flag ${flag} must be a boolean value`);
        }
    ...
    batchSize(value) {
        ...
        if (this[kOptions].tailable) {
            throw new error_1.MongoError('Tailable cursors do not support batchSize');
        }
        if (typeof value !== 'number') {
    ...
    _getMore(batchSize, callback) {
        ...
        const server = this[kServer];
        if (cursorId == null) {
            callback(new error_1.MongoError('Unable to iterate cursor with no id'));
            return;
        }
        if (server == null) {
            callback(new error_1.MongoError('Unable to iterate cursor without selected server'));
            return;
        }
...
function assertUninitialized(cursor) {
    if (cursor[kInitialized]) {
        throw new error_1.MongoError('Cursor is already initialized');
    }
}
