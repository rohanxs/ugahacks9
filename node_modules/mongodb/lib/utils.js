...
function checkCollectionName(collectionName) {
    if ('string' !== typeof collectionName) {
        throw new error_1.MongoError('collection name must be a String');
    }
    if (!collectionName || collectionName.indexOf('..') !== -1) {
        throw new error_1.MongoError('collection names cannot be empty');
    }
    if (collectionName.indexOf('$') !== -1 &&
        collectionName.match(/((^\$cmd)|(oplog\.\$main))/) == null) {
        throw new error_1.MongoError("collection names must not contain '$'");
    }
    if (collectionName.match(/^\.|\.$/) != null) {
        throw new error_1.MongoError("collection names must not start or end with '.'");
    }
    // Validate that we are not passing 0x00 in the collection name
    if (collectionName.indexOf('\x00') !== -1) {
        throw new error_1.MongoError('collection names cannot contain a null character');
    }
}
...
function executeLegacyOperation(topology, operation, args, options) {
    ...
    if (!options.skipSessions && topology.hasSessionSupport()) {
        ...
        if (opOptions == null || opOptions.session == null) {
            ...
            throw new error_1.MongoError('Use of expired sessions is not permitted');
        }
    }
...
function decorateWithCollation(command, target, options) {
    ...
    if (options.collation && typeof options.collation === 'object') {
        if (capabilities && capabilities.commandsTakeCollation) {
            ...
            throw new error_1.MongoError(`Current topology does not support collation`);
        }
    }
}
...
function getTopology(provider) {
    ...
    throw new error_1.MongoError('MongoClient must be connected to perform this operation');
}
